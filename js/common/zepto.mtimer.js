		...
		var picker = {
			init : function(){
				...
				for(var i=0; i<option.dateNum; i++){
					...
					dateStr += '<li class="'+sel+'" data-date="'+y+'-'+m+'-'+d+'">'+y+'年'+m+'月'+d+'日</li>';
				}
				dateStr += '<li></li><li></li>';
				...
				for(var j=0; j<option.timeNum; j++){
					var t = option.timeStart + 2*j,
						sel = j == 0 ? 'selected' : '';
					timeStr += '<li class="'+sel+'" data-time="'+t+':00'+-(t+2)+':00">'+t+':00'+-(t+2)+':00</li>';
							//+  '<li data-time="'+(t+2)+':00">'+(t+2)+':00'+-(t+4)+':00</li>';
					if(j==option.timeNum - 1){
						// timeStr += '<li data-time="'+(t+6)+':00">'+(t+6)+':00</li>';
					}
				}
				timeStr += '<li></li><li></li>';
			...
			renderHTML : function(){
				...
				var etime = option.timeStart + option.timeNum + 5 + ':00';
